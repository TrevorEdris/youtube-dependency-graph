...
func (a *app) getReferences(g graph.Graph, video youtube.Video, currentDepth int) ([]youtube.Video, error) {
	a.log.Info(fmt.Sprintf("======================[ %s, %s, %d ]======================\n", video.GetID(), video.GetChannelID(), currentDepth))
	a.log.Info("Base video", "title", video.GetTitle(), "channel", video.GetChannelTitle())

	parentNode, err := graph.NewNode(video.GetID())
	if err != nil {
	...
	for _, url := range urls {
		...
		childNode, err := graph.NewNode(referencedVideo.GetID())
		if err != nil {
